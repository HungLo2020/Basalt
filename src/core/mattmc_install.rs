...
pub struct MattmcInstallReport {
...
impl MattmcInstallReport {
    pub fn discovery_message(&self) -> &'static str {
        match self.discovery_result {
            DiscoverResult::Added => "Discovered MattMC and added it.",
            DiscoverResult::AlreadyExists => "MattMC game entry already exists; no changes made.",
            DiscoverResult::NotFound => "MattMC was not discovered after install.",
        }
    }
}
...
fn fetch_latest_release_tag_and_client_zip_url() -> CoreResult<(String, String)> {
    ...
    let release_json: Value = serde_json::from_str(&payload).map_err(|err| {
        CoreError::new(format!(
            "Failed to parse latest MattMC release metadata JSON: {}",
            err
        ))
    })?;
    ...
    let latest_tag = release_json
        ...
        .ok_or_else(|| CoreError::new("Latest MattMC release metadata is missing tag_name"))?
        .to_string();

    let assets = release_json
        .get("assets")
        .and_then(Value::as_array)
        .ok_or_else(|| CoreError::new("Latest MattMC release metadata is missing assets array"))?;
...
fn download_archive(archive_url: &str, destination_path: &Path) -> CoreResult<()> {
    if let Some(parent_directory) = destination_path.parent() {
        fs::create_dir_all(parent_directory).map_err(|err| {
            CoreError::new(format!(
                "Failed to prepare archive destination directory: {}",
                err
            ))
        })?;
    }
...
fn extract_zip_into_target(
    ...
    for entry_result in entries {
        let entry = entry_result.map_err(|err| {
            CoreError::new(format!(
                "Failed to read extracted MattMC ZIP entry: {}",
                err
            ))
        })?;
...
fn copy_directory_contents(source_root: &Path, target_root: &Path) -> CoreResult<()> {
    ...
    for entry_result in entries {
        let entry = entry_result.map_err(|err| {
            CoreError::new(format!("Failed to read extracted source entry: {}", err))
        })?;
...
fn temp_archive_path() -> CoreResult<PathBuf> {
...
fn temp_extraction_root() -> CoreResult<PathBuf> {
    Ok(env::temp_dir().join(format!(
...
fn unix_timestamp() -> CoreResult<u64> {
    SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|duration| duration.as_secs())
        .map_err(|err| CoreError::new(format!("Failed to read system clock: {}", err)))
}
