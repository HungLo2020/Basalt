...
use super::{artwork_cache, emulator_artwork_catalog_path, EmulationLaunchTarget, GameEntry};

#[path = "artwork/cache.rs"]
mod cache;
#[path = "artwork/download_workers.rs"]
mod download_workers;
#[path = "artwork/image_prep.rs"]
mod image_prep;
...
pub enum ArtworkDownloadResult {
    Ready {
        key: String,
        payload: PreparedArtwork,
    },
...
impl ArtworkManager {
    ...
    pub fn poll_download_results(&mut self, max_results: usize) -> Vec<ArtworkDownloadResult> {
        ...
        while results.len() < max_results {
            ...
            if let ArtworkDownloadResult::Missing { key } = &result {
                self.missing.insert(key.clone());
            } else if let ArtworkDownloadResult::Ready { key, .. } = &result {
    ...
    pub fn prepare_for_games(&mut self, games: &[GameEntry]) -> HashSet<String> {
    ...
    pub fn refresh_metadata_for_games(&mut self, games: &[GameEntry]) -> HashSet<String> {
        let _ = artwork_cache::clear_artwork_cache();
    ...
    pub fn request_for_game(&mut self, game: &GameEntry) -> Option<ArtworkRequestResult> {
        let runner = ArtworkRunnerKind::from_game(game);
        let request = runner.build_request(game)?;
        Some(self.request_artwork(request))
    }

    pub fn request_mattmc_artwork(&mut self) -> ArtworkRequestResult {
        self.request_artwork(ArtworkRequest {
    ...
    fn request_artwork(&mut self, request: ArtworkRequest) -> ArtworkRequestResult {
        ...
        if let Some(payload) = request.runner.load_builtin_artwork() {
            self.ready.insert(request.key.clone());
            return ArtworkRequestResult::Ready {
                key: request.key,
                payload,
        ...
        if self.request_download(request.clone()) {
            ArtworkRequestResult::Pending { key: request.key }
        } else {
            ArtworkRequestResult::Missing { key: request.key }
        }
    }
...
impl ArtworkRunnerKind {
    ...
    fn build_request(self, game: &GameEntry) -> Option<ArtworkRequest> {
        match self {
            ...
            Self::Emulator => Some(ArtworkRequest {
                key: format!(
                    "emulator:{}:{}",
    ...
    fn load_builtin_artwork(self) -> Option<PreparedArtwork> {
        match self {
            Self::Mattmc => image_prep::prepare_artwork_payload_from_svg_bytes(MATTMC_SVG_BYTES),
    ...
    fn has_cached_artwork(self, target: &str) -> bool {
        match self {
            Self::Steam => {
                download_workers::find_cached_steam_portrait_artwork_path(target).is_some()
            }
